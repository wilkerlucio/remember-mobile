...
(defn generate-reconciler
  ...
  [{:keys [send-queues mutation-merge] :as app} initial-state parser {:keys [pathopt migrate shared]
                                                                      :or   {pathopt true migrate nil shared nil}
  ...
  (let [rec-atom (atom nil)
        ...
        config (merge {:state       initial-state-with-locale
                       ...
                       :parser      parser
                       :shared      shared}
                 (dissoc reconciler-options :pathopt :migrate :shared))
        rec (om/reconciler config)]
    (println "reconciler config" (-> config (dissoc :send :merge-ident :merge-tree :migrate :parser)))
    (reset! rec-atom rec)
    rec))
